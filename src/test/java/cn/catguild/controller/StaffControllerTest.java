...
public class StaffControllerTest extends ApplicationTest {
	...
	public void submit() {
		for (int i = 0; i < 3; i++) {
			...
			try {
				Mono<R<Staff>> submit = controller.submit(staff);
				log.debug("成功保存：{}", submit.block().getData());
			} catch (Exception e) {
	...
	//@Test
	//public void page() {
	//	Flux<Staff> page = controller.page().getData();
	//	log.debug("职员列表：{}",page.collectList().block());
	//}
	//
	//@Test
	//public void remove() {
	//	List<String> block = controller.page().getData().map(staff -> staff.getId()).collectList().block();
	...
	//	List<Staff> block = controller.page().getData().collectList().block();
	//	assert block != null;
