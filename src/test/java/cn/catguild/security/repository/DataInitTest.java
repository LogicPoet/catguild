...
import cn.catguild.security.model.*;
 ...
 */
@Slf4j
public class DataInitTest extends ApplicationTest {
    @Autowired
    ...
    private void createRolePermissionRelation(Long roleId, Long permissionId) {
		...
		Mono<RolePermission> save = rolePermissionDao.save(rolePermission);

		log.info("角色权限关系创建结果：{}",save.block());
	}
    ...
    private Permission createPermission(String url, String name, Integer type, String permission, String method, Integer sort, Long parentId) {
		...
		Mono<Permission> save = permissionDao.save(perm);
		Permission block = save.block();
		log.info("权限创建结果：{}",block);
		return block;
    }
    ...
    private void createUserRoleRelation(Long userId, Long roleId) {
		...
		Mono<UserRole> save = userRoleDao.save(userRole);

		log.info("用户角色关系创建结果：{}",save.block());
	}
    ...
    private Role createRole(boolean isAdmin) {
		...
		Role block = roleDao.save(role).block();

		log.info("角色创建结果：{}",block);
		return block;
    }
